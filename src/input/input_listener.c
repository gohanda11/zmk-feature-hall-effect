...
// This file is just a copy of zmk/app/src/pointing/input_listener.c
// but the input processors in the main node are processed before those of the children
// and ZMK_INPUT_PROC_STOP is also respected for the children processors.

//FIXME At the moment, this cannot be used when there are other zmk,input-listener instances

#define DT_DRV_COMPAT he_input_listener
...
static int filter_with_input_config(const struct input_listener_config *cfg,
    ...
    int ret = apply_config(cfg->listener_index, &cfg->base, &data->base_processor_data, data, evt);
    if (ret < 0) {
        return ret;
